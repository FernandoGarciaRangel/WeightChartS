...
4. **`shotfull` não repinta um `<canvas>` fora do viewport.** O `#graficoPeso` sai com eixos, grelha, legenda e rótulos certos e **sem a linha** — parece que o gráfico está partido. Não está: `Chart.getChart('graficoPeso')` mostra os dados e o `borderColor` corretos. Confirmado nos dois sentidos: com `scrollIntoView` no canvas e uma pintura no viewport antes, o `shotfull` sai completo; sem isso, sai vazio. Para gráfico, use `shot` de viewport, ou role até ele primeiro.
